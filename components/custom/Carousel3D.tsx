...
export const Carousel3D = ({
  ...
  const getItemStyle = (index: number) => {
    ...
    if (diff === 0) {
      ...
      diff === -1 ||
      (diff === items.length - 1 && currentIndex === 0)
    ) {
      // Left item
      transform = `translateX(-${baseX}px) translateZ(-${baseZ}px) ${
        rotated ? "rotateY(45deg)" : ""
      }`;
      zIndex = 40;
      scale = 0.8;
      ...
      diff === 1 ||
      (diff === -(items.length - 1) && currentIndex === items.length - 1)
    ) {
      // Right item
      transform = `translateX(${baseX}px) translateZ(-${baseZ}px) ${
        rotated ? "rotateY(-45deg)" : ""
      ...
      // Far left item
      transform = `translateX(-${baseX * 2}px) translateZ(-${baseZ + 100}px) ${
        rotated ? "rotateY(60deg)" : ""
      }`;
      zIndex = 30;
      scale = 0.6;
      ...
      // Hidden items
      transform = `translateX(${baseX * 3}px) translateZ(-${baseZ + 200}px) ${
        rotated ? "rotateY(90deg)" : ""
      }`;
      zIndex = 10;
      scale = 0.4;
      ...
      <div className="relative w-full">
        ...
        >
          {items.map((item, index) => (
            <div
              key={index}
              className={`absolute overflow-hidden w-[300px] sm:w-[500px] md:w-[600px] lg:w-[700px] xl:w-[800px] rounded-2xl shadow-2xl cursor-pointer transition-all duration-700 ease-in-out ${
